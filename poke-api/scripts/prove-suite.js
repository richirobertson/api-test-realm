...
const deliberatelyBrokenResponse = "res.status(200).json({ status: 'unhealthy' });";
...
const step = (number, message) => console.log(`${pc.cyan(`${number}.`)} ${message}`);
...
if (!fs.existsSync(jestPath)) {
  console.error('Test proof could not start: install dependencies with npm install first.');
  process.exit(1);
}

if (originalSource.split(expectedResponse).length !== 2) {
  console.error('Test proof could not safely apply its known mutation; src/app.js does not have the expected health response.');
  process.exit(1);
}

let testResult;
const resultPath = path.join(os.tmpdir(), `api-test-realm-proof-${process.pid}.json`);

try {
  // Introduce one narrow, documented defect. The health test should reject this response body.
  fs.writeFileSync(applicationPath, originalSource.replace(expectedResponse, deliberatelyBrokenResponse));

  console.log();
  divider();
  console.log(pc.bold('TEST SUITE PROOF'));
  console.log('A safe, manual check that one contract test can detect a known defect.');
  divider();
  ...
  step('3', `Expect Jest to ${pc.yellow('fail')} because the contract has been broken`);
  console.log();

  testResult = spawnSync(
    process.execPath,
    [jestPath, 'test/mocked/app.test.js', '--runInBand', '--verbose', '--json', `--outputFile=${resultPath}`],
...
if (testResult.status !== 0 && jestResult.numFailedTests > 0) {
  ...
  console.log('The health-contract test rejected the deliberately incorrect response.');
  console.log('The Jest failure shown above is therefore the evidence that this test is working.');
...
console.error('The health-contract test passed even though its response was deliberately made incorrect.');
console.error('Strengthen the test before relying on it to protect this contract.');
