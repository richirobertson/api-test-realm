...
async function getRegionalEntries(client, region) {
  const regionData = await readJson(
    await client.getRegion(region),
    `Region "${region}"`,
  );
  const pokedexes = await Promise.all(
    regionData.pokedexes.map(async ({ url }) =>
      readJson(await client.getByUrl(url), "Regional Pokédex"),
    ),
  );
  ...
  return new Map(
    pokedexes
      ...
      .map(({ entry_number, pokemon_species }) => [
...
async function getCriterionCandidates(client, criterion) {
  ...
  if (typeResponse.ok) {
    ...
    return {
      kind: "type",
      name: type.name,
      candidates: type.pokemon.map(({ pokemon }) => pokemon),
    };
  }
  ...
  if (moveResponse.ok) {
    ...
    return {
      kind: "move",
      name: move.name,
      candidates: move.learned_by_pokemon,
    };
  }
...
async function findPokemonByCriterionAndRegion(client, criterion, region) {
  ...
  const candidates = [
    ...new Map(
      criterionData.candidates.map((candidate) => [candidate.name, candidate]),
    ).values(),
  ];
  const baseResults = candidates
    ...
    .map(({ name, url }) => ({
      name,
      url,
      entryNumber: regionalEntries.get(name),
    }))
    .sort(
      (left, right) =>
...
async function addMoveLearning(client, result, moveName) {
  ...
  const learning = [...grouped.values()].map(
    ({ method, level, versionGroups }) => {
      const uniqueGroups = [...new Set(versionGroups)].sort();
      const additionalVersionGroups = Math.max(uniqueGroups.length - 8, 0);
      return {
        method,
        ...(level ? { level } : {}),
        versionGroups: uniqueGroups.slice(0, 8),
        ...(additionalVersionGroups ? { additionalVersionGroups } : {}),
      };
    },
  );
  return {
...
async function addRegionalEncounters(client, result, region) {
  ...
  const areas = encounters
    ...
    .map(({ location_area, version_details }) => ({
      ...
      versions: [
        ...new Set(version_details.map(({ version }) => version.name)),
      ],
    }));
  return {
    name: result.name,
