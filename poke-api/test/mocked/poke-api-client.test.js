...
describe('PokéAPI client: deterministic transport behaviour', () => {
  it('builds the expected resource URL and returns a successful response', async () => {
    const fetchImplementation = jest.fn().mockResolvedValue(new Response('{"name":"pikachu"}', {
      status: 200,
      headers: { 'content-type': 'application/json' }
    }));
    const client = createPokeApiClient({ ...clientOptions, fetchImplementation });
  ...
  it('returns an HTTP error response without retrying it', async () => {
    const fetchImplementation = jest.fn().mockResolvedValue(new Response('Not Found', { status: 404 }));
    const client = createPokeApiClient({ ...clientOptions, fetchImplementation });
  ...
  it('surfaces a transport failure after configured retries are exhausted', async () => {
    ...
    await expect(client.getPokemon('pikachu')).rejects.toThrow('network unavailable');
    expect(fetchImplementation).toHaveBeenCalledTimes(2);
  });
});
