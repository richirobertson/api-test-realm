const {
  findPokemonByCriterionAndRegion,
} = require("../../src/pokemon-query-service");
...
function regionalClient({ typeResponse, moveResponse }) {
  return {
    ...
    getMove: jest
      .fn()
      .mockResolvedValue(moveResponse || new Response("", { status: 404 })),
    getRegion: jest.fn().mockResolvedValue(
      jsonResponse({
        pokedexes: [{ url: "https://example.test/pokedex/kanto" }],
      }),
    ),
    ...
    expect(result.results).toEqual([
      {
        ...
        imageUrl: null,
        entryNumber: 26,
        learning: [{ method: "machine", versionGroups: ["red-blue"] }],
      },
    ]);
  });
});
