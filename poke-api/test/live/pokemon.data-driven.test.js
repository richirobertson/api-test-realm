...
const validatePokemon = new Ajv({ allErrors: true, strict: false }).compile(
  pokemonSchema,
);

describe("PokéAPI data-driven Pokémon contracts", () => {
  test.each(pokemonCases)(
    "returns shared invariants for $description: $name",
    async ({ name, id }) => {
      ...
      if (!isValid) {
        throw new Error(
          `Pokémon contract failed for ${name}: ${JSON.stringify(validatePokemon.errors)}`,
        );
      }
    },
    15_000,
