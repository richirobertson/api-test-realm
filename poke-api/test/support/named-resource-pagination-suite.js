...
function registerNamedResourcePaginationSuite({
  baseUrl,
  resourcePath,
  limit,
  offset,
  ...
  async function fetchPage(url) {
    ...
    expect(body.results.every((resource) => resource.name.length > 0)).toBe(
      true,
    );
  ...
  describe(`${resourcePath} pagination`, () => {
    ...
    it("allows consumers to navigate to adjacent, non-overlapping pages", async () => {
      const { body: currentPage, names: currentNames } =
        await fetchPage(collectionUrl);
      ...
      const [{ names: previousNames }, { names: nextNames }] =
        await Promise.all([fetchPage(previousUrl), fetchPage(nextUrl)]);

      // Adjacent pages must not repeat a resource from the current page.
      expect(previousNames.some((name) => currentNames.includes(name))).toBe(
        false,
      );
